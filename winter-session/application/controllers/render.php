	...
	function listAllDepartments(){
		...
		try{
			...
			foreach ($departments as $key => $value) {
				$list_items .= "<li>
									<a href='winter-session/courses/results/?department={$value['department_code']}'>{$value['department_name']}</a>
	...
	function getSessionCodeSelectOptions(){
		...
		try{
			...
			foreach ($session_codes as $key => $value) {
				$option_items .= "<option value='{$key}' ";
				if($key=='WTR') { $option_items .= "selected=\"selected\" "; }
				$option_items .= ">{$value}</option>";
			}
