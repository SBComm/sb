...
(function($){
		...
		create: function(el, options) {
			...
			if(this.o.filter == true || this.o.controls == true){
				...
				if(this.o.filter == true){
					x += '<ul id="dcsns-filter" class="option-set filter clearfix">';
					x += '<li class="filter-label"><span>View:</span></li>';
					x += this.o.wall == true ? '<li><a href="#filter" data-group="dc-filter"  data-filter="*" class="selected link-all iso-active">all</a></li>' : '' ;
					var $f = $('.filter',el);
					$.each(opt.feeds, function(k,v){
						//2014.04.29 Josh Palmeri - use Font icons instead of image
						//FontAwesome: http://fortawesome.github.io/Font-Awesome/icons/
						/*x += v.id != '' ? '<li class="active f-'+k+'"><a href="#filter" rel="'+k+'" data-group="dc-filter" data-filter=".dcsns-'+k+'"><img src="'+opt.iconPath+opt.feeds[k].icon+'" alt="" /></a></li>' : '' ;*/
						x += v.id != '' ? '<li class="active f-'+k+'"><a href="#filter" rel="'+k+'" data-group="dc-filter" data-filter=".dcsns-'+k+'"><i class="font-icon fa fa-'+ opt.feeds[k].iconFont +'"></i></a></li>' : '' ;
					});
					x += '</ul>';
				}
					...
					$.each(a, function(i,item){
						if(i < n){
							...
							switch(type)
							{
								...
								q = href;
								y = '<a href="'+q+'" class="thumb" target="_blank"><img src="'+ua+'" alt="" /></a>' ;
								z = '<span class="twitter-user"><a href="https://www.twitter.com/'+un+'"><strong>'+item.user.name+' </strong>@'+un+'</a></span>';
								z += linkify(item.text);
								if(o.images != '' && item.entities.media){
									$.each(item.entities.media, function(i,media){
										z += '<a href="'+media.media_url_https+'"  target="_blank" target="_blank"><img src="'+media.media_url_https+':'+o.images+'" alt="" /></a>';
									});
								}
								sq = item.id_str;
								break;
								...
								// 2014.04.28 Josh Palmeri fix for IE8 showing Updated NaN years + NaN months ago
								/*
								case 'flickr':
								...
								*/
								...
								case 'flickr':
								var d = new Date();
								d = d.setRFC3339(parseTwitterDate(item.published));
								x =	item.title;
								y = '<a href="' + q + '" title="'+ item.title +'"><img src="' + item.media.m + '" alt="" /></a>';
								break;
							...
							icon = '<a href="'+href+'"><i class="font-icon fa fa-'+ o.iconFont +'"></i></a>';
							...
							$.each(o.out.split(','), function(i,v){
								...
								switch(v)
								{
									case 'intro':
									if(type == 'twitter'){
										zintro = '<span class="section-'+v+'"><a href="'+q+'" target="_blank">'+decodeURIComponent(intro)+'</a> <span><a href="https://twitter.com/'+un+'/status/'+item.id_str+'" target="_blank">'+nicetime(new Date(d).getTime(),0)+'</a></span></span>';
									} else {
										zintro = '<span class="section-'+v+'"><a href="'+q+'" target="_blank">'+decodeURIComponent(intro)+'</a> <span>'+nicetime(new Date(d).getTime(),0)+'</span></span>';
									}
									break;
									case 'title':
									zz += x;
									...
									if(type == 'rss'){
										...
										y = src ? '<a href="'+q+'" class="thumb" target="_blank"><img src="'+src+'" alt="" /></a>' : '' ;
									}
									zz += y;
									break;
									case 'text':
							...
							var out = '<li rel="'+ob+'" class="clearfix dcsns-li dcsns-'+type+' dcsns-feed-'+fn+'">'+w+'<div class="inner">'+zz+'<span class="clear"></span></div>'+zintro+icon+'</li>', str = opt.remove;
	...
	function linkify(text){
		...
		text = text.replace(/(^|\s)#(\w+)/g, '$1#<a href="http://twitter.com/search/%23$2" target="_blank">$2</a>');
		return text;
	}
