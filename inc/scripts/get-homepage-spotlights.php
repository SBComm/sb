	...
	foreach ($posts as $post) {
		...
		$html .= '
	        <div class="boldMovesBox gtm-home-spotlight" style="background-image: url(\''.$spotlight_image.'\')">
				<a href="'.$post_link.'" class="boldMovesBox-title" title="Read '.$post->acf->homepage_spotlight_title.'">
					<h3>'.$post->acf->homepage_spotlight_title.' <i class="fa fa-angle-right"></i></h3>
				</a>
				<a href="'.$post_link.'" class="boldMovesBox-story" title="Learn about how '.$post->acf->homepage_spotlight_sub.'">
				...
				<a href="'.$cat_link.'" class="boldMovesBox-link gtm-category-link" title="See all stories in '.$spotlight_category.'"><span>All </span>'.$spotlight_category.'<span> News <i class="fa fa-long-arrow-right"></i></span></a>
			</div>
		';
	}
