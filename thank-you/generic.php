...
<!--[if gt IE 8]><!--> <html class="no-js" lang="en"> <!--<![endif]-->
	...
	<!--site variables-->
		<?php
		    ...
		    $form_id = !empty(urlencode($_GET["id"])) ? $a : "1FAIpQLSdJSrkA73go1tXx3GqLXN9u3clup2YqGBsj24a3Xa1RViRFDQ";
		?>
							...
							<?php 
                                ...
                                if ($resp->isSuccess()) {
                                	if($_POST['first-name']!='' && $_POST['last-name']!='' && $_POST['email']!='' && $_POST['date']!='' && $_POST['topic']!='' && $_POST['g-recaptcha-response']!='') {
									    ...
									    $url = 'https://docs.google.com/forms/d/e/'.$form_id.'/formResponse?entry.400857025='.htmlspecialchars($_POST['first-name']).'&entry.417680940='.htmlspecialchars($_POST['last-name']).'&entry.1523563919='.htmlspecialchars($_POST['email']).'&entry.1787604701='.htmlspecialchars($_POST['date']).'&entry.818029876='.htmlspecialchars($_POST['topic']).'&entry.1881785626='.htmlspecialchars($_POST['source-url']).'&submit=Submit';
										...
										include($path . 'content/forms/submit_google_generic_2017_form.php');

									} else {
										include($path . 'content/forms/re-submit_google_generic_2017_form.php');
									}
                                } else {
