 ...
 *
 * See http://web.archive.org/web/20120920065217/http://api.jquery.com/category/plugins/templates/ 
 */
...
/* From admissions old site */

var code = decodeURIComponent($.urlParam('code'));

var NA_MSG = "<strong>Information currently unavailable.</strong>";


function fillUp(element,data){
    return $('<div/>').html(data);          
}

function inactivate(){
    $("div.active").removeClass("active").addClass("hide-accessible");
...
$(document).ready(function() {
    if(code!=''&&code!='null'){
        ...
        $.getJSON('http://admissions.cc.stonybrook.edu/mobile/program-ugadmissions-sbc.php?p='+code+'&callback=?', function(data) {
            ...
            $("#loading").hide();
                ...
                $('.contact').on('click',function(){
                    ...
                    $("div.contact").removeClass("hide-accessible").addClass("active");
                });

                $("a.home").on('click',function(){
                    inactivate();
                    $(this).addClass("active");
                    $("div.description").removeClass("hide-accessible").addClass("active");
                });

                $("a.degree").on('click',function(){
                    $("a.degree").addClass("active");

                    var icon = $(this).find(".fa");
                    
                    icon.removeClass("hide-accessible");

                    if( $('div.degree').children().length ){
                        inactivate();
                        $("a.degree").addClass("active");
                    ...
                    $.getJSON('http://admissions.cc.stonybrook.edu/mobile/program-ugadmissions-sbc.php?p='+code+'&require=1'+'&callback=?', function(data) {
                        ...
                        if( data.hasOwnProperty('is_error') ) {
                            ...
                            $('div.degree').html(data.requirements);
                        }
                    ...
                    $("#inside-main").css('height','auto');     
                    
                });

                $("a.schedule").on('click',function(){
                    $("a.schedule").addClass("active");

                    if( $('div.schedule').children().length ){
                        inactivate();
                        $("a.schedule").addClass("active");
                        $("div.schedule").removeClass("hide-accessible").addClass("active");
                        return false;               
                    }
                ...
                $("a.courses").on('click',function(){
                    $("a.courses").addClass("active");
                    
                    if( $('div.courses').children().length ){
                        inactivate();
                        $("a.courses").addClass("active");
                        $("div.courses").removeClass("hide-accessible").addClass("active");
                        return false;               
                    }
        ...
        $('#container').html('Oops! Hmm... How\'d you get here? <br /><br />Try going back from whence you came, or <a href="undergraduate-admissions/academics/">view all academic programs.</a><br /><br />');
    }
